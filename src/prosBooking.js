...
const displayBooking = async (bookingPromise) => {
    ...
    if (bookListArr.length === 0) {
        let noBookingsMessage = "<div class='booking-history-customer me-5 ms-5'><p>No bookings found</p></div>";
    ...
    bookListArr.forEach((x) => {
        ...
        let serviceAddress = x.where === "onhome" ? '<p>Customer will visit your place</p>' : `<a href="#" class="serviceAddressButton" data="${x.address}">Click to see the address</a>`;
...
let baseHTML = `
    ...
    if (pendingDisplay === "") {
        pendingTab.innerHTML = "<div class='booking-history-customer me-5 ms-5'><p>No bookings found</p></div>";
    } else {
        pendingTab.innerHTML = pendingDisplay;
    }

    if (confirmedDisplay === "") {
        confirmedTab.innerHTML = "<div class='booking-history-customer me-5 ms-5'><p>No bookings found</p></div>";
    } else {
        confirmedTab.innerHTML = confirmedDisplay;
    }

    if (cancelledDisplay === "") {
        cancelledTab.innerHTML = "<div class='booking-history-customer me-5 ms-5'><p>No bookings found</p></div>";
    } else {
...
document.getElementById('pendingTabButton').addEventListener('click', function () {
    pendingTab.style.display = "block";
    confirmedTab.style.display = "none";
    cancelledTab.style.display = "none";
...
document.getElementById('confirmedTabButton').addEventListener('click', function () {
...
document.getElementById('cancelledTabButton').addEventListener('click', function () {
    pendingTab.style.display = "none";
    confirmedTab.style.display = "none";
